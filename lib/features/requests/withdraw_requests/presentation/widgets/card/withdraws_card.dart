...
import 'package:alfa_dashboard/features/requests/withdraw_requests/presentation/widgets/card/build_detail_row.dart';
...
class WithdrawsCard extends StatelessWidget {
  WithdrawsCard({super.key});
  ...
  Widget bottomData(BuildContext context) {
    return BlocConsumer<WithdrawsCubit, WithdrawsState>(
      listener: (context, state) {
        if (state is WithdrawsError) {
              ...
              children: [
                ...
                if(state.transactions.isEmpty)
                  SizedBox(
                    height: 50,
                    child: Center(
                      child: Text(AppStrings.withdrawsIsEmpty, style: TextStyle(color: AppConstants.clrSmallText),),
                    ),
                  )
                else
                ListView.builder(
